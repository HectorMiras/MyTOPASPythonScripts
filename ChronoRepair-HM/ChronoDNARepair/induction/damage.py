...
import os
from ChronoDNARepair.induction.SDD import *
...
class DamageToDNA:
    ...
    def readSDDAndDose(self, path, namessd = 'DNADamage_sdd.txt', namephsp = 'DNADamage.phsp', version='2.0', particleTime=0, lesionTime=0, doseperexposure=1):
        if namephsp is not None:
            dosepath = os.path.join(path, namephsp)
            f = open(dosepath, 'r')
            lines = f.readlines()
            doseinthisexposure = 0.0
            for l in lines:
                split = l.split()
                self.doses = np.append(self.doses, float(split[1])-doseinthisexposure)
                self.accumulateDose += self.doses[-1]
                self.damagespertrack['Dose'].append(self.doses[-1])
                doseinthisexposure += self.doses[-1]
                if doseinthisexposure >= doseperexposure:
                    doseinthisexposure = 0.0
        sddpath = os.path.join(path, namessd)
    ...
    def readFromSDD(self, path, version='2.0', particleTime=0, lesionTime=0):
        ...
        for d in reader.damages:
            ...
            self.damagespertrack['DSB'][self.numtracks-1] += dsite.isthereDSB
        ...
        for id, damage in enumerate(self.damageSitesAtCurrentTime):
            ...
            if (id < len(self.damageSitesAtCurrentTime) - 1 and self.damageSitesAtCurrentTime[id + 1].newExposure >= 1) or id == len(self.damageSitesAtCurrentTime) - 1:
                ...
                if recalculatePerEachTrack or id == len(self.damageSitesAtCurrentTime) - 1:
                    ...
                    dosefromlastrecalculation = 0
                if len(self.doses) > 0:
                    self.cumulativeDose += self.doses[iEvent]
                    dosefromlastrecalculation += self.doses[iEvent]
    ...
    def classifyDamageSites(self):
        ...
        for iCh, damageScoreCh in damageScore.items():
            ...
            while maxDamage > minDamage and len(basePairs) != 0:
                ...
                self.provnsites += 1
                complexity = self._transformMaxDamageIntoComplexityScore(maxDamage)
                self.assignComplexities(iCh, index, complexity)
            ...
            if iChr in self.BDMap.keys():
                if iBp + i in self.BDMap[iChr].keys():
                    if 1 in self.BDMap[iChr][iBp + i].keys():
                        if not self.BDMap[iChr][iBp + i][1].complexity or damage > self.BDMap[iChr][iBp + i][1].complexity:
                            self.BDMap[iChr][iBp+i][1].complexity = damage
                    if 2 in self.BDMap[iChr][iBp + i].keys():
                        if not self.BDMap[iChr][iBp + i][2].complexity or damage > self.BDMap[iChr][iBp + i][2].complexity:
            ...
            if iChr in self.SSBMap.keys():
                if iBp + i in self.SSBMap[iChr].keys():
                    if 1 in self.SSBMap[iChr][iBp + i].keys():
                        if not self.SSBMap[iChr][iBp + i][1].complexity or damage > self.SSBMap[iChr][iBp + i][1].complexity:
                            self.SSBMap[iChr][iBp+i][1].complexity = damage
                    if 2 in self.SSBMap[iChr][iBp + i].keys():
                        if not self.SSBMap[iChr][iBp + i][2].complexity or damage > self.SSBMap[iChr][iBp + i][2].complexity:
            ...
            if iChr in self.DSBMap.keys():
                if iBp + i in self.DSBMap[iChr].keys():
                    if 1 in self.DSBMap[iChr][iBp + i].keys():
                        if not self.DSBMap[iChr][iBp + i][1].complexity or damage > self.DSBMap[iChr][iBp + i][1].complexity:
                            self.DSBMap[iChr][iBp+i][1].complexity = damage
                    if 2 in self.DSBMap[iChr][iBp + i].keys():
                        if not self.DSBMap[iChr][iBp + i][2].complexity or damage > self.DSBMap[iChr][iBp + i][2].complexity:
    ...
    def outputSDDFile(self, filename, version='2.0'):
        damageSitesPerTime = self.classifiedDamage
        numSites = 0
        firstExposure = True
        newEvent = True
        with open(filename, 'a') as f:
            for iCh in damageSitesPerTime.keys():
                ibpsTakenForThisChromosome = []
                for i in range(len(damageSitesPerTime[iCh])):
                    initialBpId = damageSitesPerTime[iCh][i]
                    dir = 0; indir = 0
                    bd = 0; sb = 0; dsb = 0
                    for j in range(self.nbpForDSB):
                        if initialBpId + j not in ibpsTakenForThisChromosome:
    ...
    def getComplexityDistribution(self, plot=True, savefile='', fromRecomputedSites=True, density=False):
        ...
        if fromRecomputedSites:
            ...
            for damage in self.damageSitesAtCurrentTime:
                if damage.isthereDSB > 0:
                    numdamages = damage.numberofbasedamages + damage.numberofstrandbreaks
                    n = np.append(n, numdamages)
        complexities = np.arange(2, 16)
        complexitydist = np.zeros(14)
        for i in range(len(complexities)):
            complexitydist[i] = np.sum(n == complexities[i])
        if density:
            complexitydist = complexitydist / np.sum(complexitydist)
        if plot:
            fig, ax = plt.subplots(1, 1, figsize=(8, 6))
            ax.bar(complexities, complexitydist, width=0.5, color='blue', edgecolor='black', alpha=0.5)
    ...
    def produce3DImage(self, show=True, microscopePSFWidth = 0.4, resolution = 0.4, xmin = -5, xmax = 5, ymin = -5, ymax = 5, zmin = -5, zmax = 5,
        ...
        img = ax.scatter(x, y, z, c=v, cmap='viridis', marker='s', s=20, vmax=1)
        fig.colorbar(img)
        if show:
            plt.show()
    ...
    def produce2DImages(self, microscopePSFWidth = 0.2, resolution = 0.1, xmin = -5, xmax = 5, ymin = -5, ymax = 5, saveFile=None, onlyz=False, title=None, useSites=False):
        ...
        for id in range(len(ids1)):
            ...
            ax.imshow(img2d, cmap='viridis', extent=[xmin, xmax, ymin, ymax], origin='lower', vmax=1)
            if title is None:
    ...
    def SetUpColorMap(self):
        ...
        map_object = LinearSegmentedColormap.from_list(name='viridis', colors=color_array)
        try:
            plt.register_cmap(cmap=map_object)
        except:
            pass

    def _transformMaxDamageIntoComplexityScore(self, maxDamage):
        num_dsb = (maxDamage + 1e-6) // 10.0
        remaining = np.round(maxDamage - num_dsb * 10.0, 3)
        num_ssb = (remaining + 1e-6) // 0.1
        remaining = np.round(remaining - num_ssb * 0.1, 3)
        num_bd = (remaining + 1e-6) // 0.001
        if num_dsb < 1:
            return 1
        else:
            return int(2 * num_dsb + num_ssb + num_bd)
